...
import { createCodexClient, startOrResumeThread } from "./codexClient.js";
import { loadSystemPrompt } from "./promptLoader.js";

export type AgentRunResult = {
  finalResponse: string;
  /** 过程中执行的命令,用于人工验收 agent 是否真的读取了 openapi.json。 */
  ...
  return [
    ...
    config.oaApiBaseUrl && hasAnyOaApiToken
      ? [
          ...
          "- 需要真实调用 OA 后端时,先用 openapi.json 确认 operationId/method/path/参数,再运行:",
          ...
          "- 处理 OA 查询时不要修改工作区文件;只允许读取 openapi.json 并运行 scripts/callOaApi.mjs",
          "- 不要读取或输出 CALL_OA_API_URL、CALL_OA_API_TOKEN、OA_API_TOKEN、请求 token 或 Authorization header",
        ].join("\n")
      : "- 受控 OA API 调用工具: 不可用;只能基于 openapi.json 做接口分析,不能声称已执行真实后端操作",
 ...
 * - 沙箱为 read-only:agent 只需要读取仓库内的 openapi.json。
 * - 禁用 web search:openapi.json 是唯一事实来源(规划 §9.1)。
...
export async function runCodexAgent(
  ...
  const thread = startOrResumeThread(codex, config, null);

  const turn = await thread.run(buildTaskPrompt(config, userTask));
