...
test("uses the automation BFF for list and audit requests", async () => {
  ...
  try {
    ...
    await getAutomationRun("run/unsafe")
    ...
    assert.equal(requests[1], "/api/automation/runs/run%2Funsafe?include=projects%2Cai_interactions%2Cattempts%2Cweekly_report_pending_items")
    assert.equal(requests[2], "/api/automation/runs/run%2Funsafe?include=attempts")
    assert.equal(requests[3], "/api/automation/runs/run%2Funsafe/trace-events")
  } finally {
    globalThis.fetch = originalFetch
  }
})
