...
输出必须引用底稿和每条当天总结，不能出现思考过程或无依据的新功能。独立审核检查遗漏、状态冲突、重复及个人贡献归属。全文最多 1000 字；输入超出预算不截断。生成或审核失败只重试当前作者这份周报（使用任务配置的总次数和间隔），失败草稿与审核意见反馈下一轮修订；耗尽后保留原文。审核依赖模型，不能保证修复底稿中原有的所有错误。
...
本任务的 `retry_max_attempts` / `retry_interval_seconds` 控制仓库总结、周报生成各自的步骤重试；总次数包含首次调用。周报一轮含生成和审核，审核通过才保存；失败不会再次读取或总结已成功仓库。401/403、账号缺失、权限或配置问题、非法工具调用等直接终止当前步骤，无整任务自动重跑。

`automation_ai_interactions.response_payload_sanitized.attempts` 保留每轮拒绝的草稿、原始生成/审核响应（脱敏且有长度上限）、具体 review_issues、原因、是否继续重试和请求 ID。错误后恢复成功时，这些失败项仍然保留；不需要改 OA 后端或数据库结构。账号缺失导致模型尚未调用时，原因见项目 warnings / Trace。
