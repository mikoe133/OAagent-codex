...
`Retry-After`，缺失时使用带抖动的指数退避。额度、余额或消费上限类 `429`
不会被当作瞬时限流追加退避。每次 `429` 会输出一条脱敏的
`[model-relay]` 结构化日志，包含 provider、错误 code、等待时间和上游
request ID，不包含 prompt、回答正文或 API key。
...
不会根据普通业务词语推断目录，也不会在路由失败时暴露知识库写接口。唯一的显式字符触发规则是 `RWKV`（不区分大小写）：
命中时始终把 `rwkv_knowledge` 固定资料模块放在路由首位，同时保留模型选择或安全降级得到的其他接口域。
