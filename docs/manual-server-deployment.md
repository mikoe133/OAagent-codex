...
| Agent 监听地址 | `192.168.251.1:3003` | `192.168.251.1:3011` |
| Web 监听地址 | `192.168.251.1:3001` | `192.168.251.1:3010` |
...
AGENT_PORT=3003
WEB_BIND_ADDRESS=192.168.251.1
WEB_PORT=3001
```
...
`192.168.251.1` 是当前服务器的 `docker0` 地址,用于让同机但位于其他 Docker 网络的 OA 后端访问测试 Agent。换服务器后用 `ip -4 addr show docker0` 重新确认。测试服务器的 `3002` 端口由 Alphachain/OA Node 服务使用，因此测试 Agent 固定使用 `3003`。生产环境将 `COMPOSE_PROJECT_NAME` 改为 `oa-agent-prod`，保持 `AGENT_BIND_ADDRESS` 和 `WEB_BIND_ADDRESS` 为 `192.168.251.1`，将 `AGENT_PORT` 改为 `3011`，将 `WEB_PORT` 改为 `3010`，并使用生产环境自己的 SSO 与自动化密钥。不要把 `.env` 上传到 Git 或源码压缩包。
...
curl -fsS "http://192.168.251.1:$WEB_PORT/login" >/dev/null
