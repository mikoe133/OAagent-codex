...
test("publishes release images and transfers private deployment artifacts", async () => {
  ...
  assert.equal(workflow.match(/PROJECT_PROGRESS_GITHUB_APP_ID: \$\{\{ vars\.PROJECT_PROGRESS_GITHUB_APP_ID \}\}/g)?.length, 2)
  ...
  assert.equal(workflow.match(/^      AGENT_BIND_ADDRESS: 192\.168\.251\.1$/gm)?.length, 2)
  assert.equal(workflow.match(/^      WEB_BIND_ADDRESS: 192\.168\.251\.1$/gm)?.length, 2)
  assert.doesNotMatch(workflow, /vars\.(?:AGENT|WEB)_BIND_ADDRESS/)
  ...
  assert.equal(workflow.match(/OA_DOCKER_API_BASE_URL OA_KNOWLEDGE_BASE_API_KEY OA_AGENT_SSO_SHARED_SECRET OA_AGENT_SSO_TTL_SECONDS OA_AGENT_AUTOMATION_TOKEN DATABASE_URL OA_SESSION_SECRET OA_PROJECT_SYNC_TOKEN PROJECT_PROGRESS_GITHUB_APP_ID PROJECT_PROGRESS_GITHUB_APP_PRIVATE_KEY; do/g)?.length, 2)
...
test("does not expose worker-only credentials to the agent service", async () => {
  ...
  assert.doesNotMatch(agentService, /OA_PROJECT_SYNC_TOKEN|PROJECT_PROGRESS_GITHUB/)
