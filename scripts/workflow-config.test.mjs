...
test("publishes release images and transfers private deployment artifacts", async () => {
  ...
  assert.equal(workflow.match(/OA_KNOWLEDGE_API_KEY: \$\{\{ secrets\.OA_KNOWLEDGE_API_KEY \}\}/g)?.length, 2)
  ...
  assert.equal(workflow.match(/OA_DOCKER_API_BASE_URL OA_KNOWLEDGE_API_KEY OA_AGENT_SSO_SHARED_SECRET OA_AGENT_SSO_TTL_SECONDS OA_AGENT_AUTOMATION_TOKEN DATABASE_URL OA_SESSION_SECRET OA_PROJECT_SYNC_TOKEN PROJECT_PROGRESS_GITHUB_TOKEN; do/g)?.length, 2)
