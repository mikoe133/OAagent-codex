...
test("upgrades existing weekly report audit columns with a default for older writers", async () => {
  ...
  assert.match(migration, /SELECT column_name, column_default AS default_value/)
  assert.match(migration, /projectWeeklyReportSyncColumnRows\[0\]\?\.default_value == null/)
